...
class ZigGenerator(GeneratorBase):
    ...
    def generate(self, path: pathlib.Path, *, is_exclude_function=None, custom: Optional[TYPE_CALLBACK] = None, return_byvalue_workaround=False) -> List[str]:
        self.custom = custom
        self.texts = [
...
''')
    ...
    def write_function(self, f: FunctionCursor, return_byvalue_workaround=False) -> Optional[str]:
        args = [
            f'{rename_symbol(param.name)}: {self.zig_type(param, True)}' for param in f.params]
        if f.is_variadic:
            args.append('...')
