...
class ZigGenerator(GeneratorBase):
    ...
    def generate(self, path: pathlib.Path, *, function_custom: Optional[Callable[[FunctionCursor], bool]] = None, is_exclude_function=None, custom: Optional[TYPE_CALLBACK] = None):
        self.custom = custom
        self.texts = [
        ...
        for header in self.headers:
            ...
            for f in self.parser.functions:
                ...
                workaround = False
                if function_custom:
                    workaround = function_custom(f)
                self.write_function(f, workaround=workaround)
...
''')
    ...
    def write_function(self, f: FunctionCursor, workaround=False):
        args = [
            f'{rename_symbol(param.name)}: {self.zig_type(param, True)}' for param in f.params]
        if f.is_variadic:
            args.append('...')
